...
protected:
  ...
  {
    if constexpr (std::is_same<ClientType, rclcpp::GenericClient>::value) {
      return GenericClientSendEmptyRequestAndWait(timeout);
    } if constexpr (std::is_same<ClientType, rclcpp::Client<test_msgs::srv::Empty>>::value) {
    ...
    const rclcpp::QoS & qos = rclcpp::ServicesQoS())
  {
    ...
    } else if constexpr (std::is_same<ClientType, rclcpp::Client<test_msgs::srv::Empty>>::value) {
...
public:
  ...
  {
    (void)idx;
    if (std::is_same<T, rclcpp::Client<test_msgs::srv::Empty>>()) {
      return "Client";
    }

    if (std::is_same<T, rclcpp::GenericClient>()) {
...
TYPED_TEST_SUITE(TestAllClientTypesWithServer, ClientTypes, ClientTypeNames);
...
TYPED_TEST(TestAllClientTypesWithServer, async_send_request_callback_with_request)
{
  ...
  if (std::is_same<ClientTypes, rclcpp::GenericClient>::value) {
        GTEST_SKIP() << "Skipping test for rclcpp::GenericClient";
  }
}

TYPED_TEST(TestAllClientTypesWithServer, client_qos_depth)
{
  using ClientTypes = TypeParam;
